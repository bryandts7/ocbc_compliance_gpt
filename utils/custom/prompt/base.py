...
When writing the filter, you should only use the attributes inside the list:
['type_of_regulation', 'sector']

DO NOT USE ANY OTHER ATTRIBUTES!

Notes for each filter:
`type_of_regulation` is either `Surat Edaran`, `Peraturan Bank Indonesia`, or `Peraturan Anggota Dewan Gubernur`
`sector` is either `Sistem Pembayaran Dan Pengelolaan Uang Rupiah`, `Makroprudensial`, `Moneter`, or `Perbankan`
