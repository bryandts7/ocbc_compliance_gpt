...
def get_retriever_bi_ojk(vector_store: VectorStore, llm_model: BaseLanguageModel, embed_model: Embeddings, top_n: int = 7, top_k:int = 20, config: dict = {}):
...
def get_lotr_bi_ojk(vector_store_bi: VectorStore, vector_store_ojk: VectorStore, llm_model: BaseLanguageModel, embed_model: Embeddings, top_n: int = 7, top_k:int = 20, config: dict = {}):
    retriever_bi = get_retriever_bi_ojk(vector_store=vector_store_bi, llm_model=llm_model, embed_model=embed_model, top_n=top_n, top_k=top_k, config=config)
    retriever_ojk = get_retriever_bi_ojk(vector_store=vector_store_ojk, llm_model=llm_model, embed_model=embed_model, top_n=top_n, top_k=top_k, config=config)
